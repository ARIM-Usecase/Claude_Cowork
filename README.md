...
- **Python基礎:** 変数、リスト、for文、関数の呼び出しを理解している方
- **データ分析ライブラリ:** pandas・numpy の基本操作（DataFrame操作、配列処理）
- **機械学習の基礎:** scikit-learn における以下の概念の理解
  - 主成分分析（PCA）の考え方
  - クラスタリング（K-means）の基本
  - 訓練・テストデータ分割と交差検証の重要性
  - モデル評価指標（精度・MAE・R2など）
- **X線回折（XRD）の基本知識:** 以下の概念を理解していることが強く推奨されます
  - Braggの法則（nλ = 2d sinθ）
  - d値（結晶面間隔）の意味
  - XRDパターンと結晶構造の関係
  - 時間分解（その場観察）測定の特徴
...
本教材は「AIで書かせたコードを学ぶ」のではなく、**コードが物理的・化学的に妥当であるかを、基礎知識をもとに自分たちで検証する**ことを目的としています。

各ノートブックには、データ解析の各段階で専門家（実験実施者・計算化学者）への質問コメント欄が設けられています。これらは「このコードの選択は正しいのか」「このグラフから何が読み取れるのか」を確認する場です。

**質問・コメント欄への例:**
- 「このスムージング方法（Savitzky-Golay）は放射光XRDで使われていますか？」
- 「PC1とPC2の意味は何ですか？」
- 「このクラスタリング結果は物理的に意味のあるフェーズ分け になっていますか？」

### 前提知識がない場合

- 各ノートブックの冒頭に「対象読者・前提知識・動作環境」ブロックを記載しています
- 各セクションで専門用語の説明を加えています
- わからない部分は遠慮なく質問・指摘してください
...
本リポジトリに含まれるデータセット（`data/` フォルダ内）は、**2026年10月1日のワークショップのみの公開**です。

- **公開期間:** 2026年9月24日～10月1日
- **用途:** ARIM×Claude実践編セミナーの参加者向け教材
- **削除予定日:** 2026年10月1日（ワークショップ開催日）

ワークショップ開催後、データファイルは本リポジトリから削除されます。長期的にコードの参考にしたい場合は、ワークショップ開催前に、このリポジトリを個人のマシン / ローカル環境に保存してください。

データの利用条件は、ARIMデータポータル利用規約に従います（詳細は本セクション下の「データセット情報」を参照）。
...
ワークショップ開催後（2026年10月1日以降）、リポジトリから `data/` フォルダが削除されます。

**その後の利用について:**
- **コードの参考にしたい場合:** 本リポジトリは削除後も存続し、ノートブック（コード）はそのまま利用可能です。ただし、ノートブック冒頭の「教材への接続」セルは `git clone` しても `data/` フォルダが存在しないため、実行時に「データなし」エラーが出ます。
- **同じデータで実行したい場合:** ワークショップ前に個人マシンにクローン・保存したものをご利用ください。
- **新規にデータを使用したい場合:** ARIMデータポータルから課題番号JPMXP1222QS0007の公開データセットを利用申請してください（https://doi.org/10.71947/arim.jpmxp1222qs0007）。

**削除の確認:** 2026年10月1日以降にリポジトリをご確認いただき、`data/` フォルダが削除されたことを確認できます。
